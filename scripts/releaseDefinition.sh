...
sfp releasedefinition:generate -n "$RELEASE_NAME" -b "$BRANCH" -d "$DIRECTORY" -f "$CONFIG_FILE" -c "$SOURCE_TAG" | tee release-definitions/out.log
sed 's/\x1b\[[0-9;]*m//g' release-definitions/out.log > release-definitions/cleaned_out.log
cat release-definitions/cleaned_out.log | sed -n '/release: release/,/workItemUrl: https:\/\/jira.apps.eop.gov\/browse/p' > release-definitions/release.yml
